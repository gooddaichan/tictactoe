...
mat = np.loadtxt("data/tictactoe4_new.csv", dtype=None,skiprows=1,delimiter=",")
ind_train = np.random.choice(9577220, 40,replace=False)
ind_test = np.array([i for i in range(9577220) if i not in ind_train])

x_train = mat[ind_train,:-1]
x_test = mat[ind_test,:-1]
y_all = np.zeros([len(mat), 4])
...
with tf.Graph().as_default() as g:
    ...
    x_ph = tf.placeholder(tf.float32, [None, 16])
    y_ph = tf.placeholder(tf.float32, [None, 4])
